...
function unescape($string, $euro=false) {
   $result = "";
   for ($i = 0; $i < strlen($string); $i++) {
       $decstr = "";
       for ($p = 0; $p <= 5; $p++) {
       ...
       list($decodedstr, $num) = unescapebycharacter($decstr, $euro);
       $result .= urldecode($decodedstr);
       $i += $num ;
   }
   return $result;
...
function unescapebycharacter($str, $euro) {

   $char = $str;

	if ($euro)
   		if ($char == '%u20AC') { return array("&euro;", 5); }
   	else
