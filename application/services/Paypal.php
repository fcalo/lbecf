...
class Service_Paypal {
	...
	*/
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------
	*/
	public function CallPreapproval( $notifyUrl,$returnUrl, $cancelUrl, $currencyCode, $startingDate, $endingDate, $maxTotalAmountOfAllPayments,
								...
								$maxAmountPerPayment, $maxNumberOfPaymentsPerPeriod, $pinType )
	{
		...
		*/
		...
		// required fields
                ...
                $nvpstr = "ipnNotificationUrl=" . urlencode($notifyUrl) ;
		$nvpstr .= "&returnUrl=" . urlencode($returnUrl) . "&cancelUrl=" . urlencode($cancelUrl);
	...
	*/
	private function hash_call($methodName, $nvpStr)
	{
		...
		$errorLanguage	= urlencode("es_ES");		// This should be the standard RFC 3066 language identification tag, e.g., en_US
        ...
        public function getCmdPreapproval($preapprovalKey){
            return "cmd=_ap-preapproval&preapprovalkey=".$preapprovalKey;
        }
	...
	----------------------------------------------------------------------------------
	*/
	public function getUrlPayPal ( $cmd )
	{
